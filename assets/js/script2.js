...
function listClickManager(dataFromJS) {
  ...
  if (dataFromJS.target.className === "editButton") {
    ...
    acceptManager($span, $editForm, $input, $edit_button);
...
function editManager($span, $editForm, $input, $edit_button) {
...
function create_Task(title) {
  ...
  $list_item.setAttribute("data-id", id_value);
  // span for text
...
function updateToDo(post_ID) {
  const putToDo = new XMLHttpRequest();
  const url_id = "http://195.181.210.249:3000/todo/" + post_ID;
  console.log(post_ID);
...
function postToDo() {
  ...
  sendToDo.send("title='pierwsza proba dodania'&author=AreG&description=" + $input_content.value);
